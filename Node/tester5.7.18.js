...
//rest api to get a record by id for given table
//example: select * from ampms WHERE ampmsId = 132;
app.get('/getRecordFor/:id/idName/:idName/tableName/:tableName', function (req, res) {
   var thisId     = req.params.id;
   var thisTable  = req.params.tableName;
   var thisIdName = req.params.idName;
   connection.query('select * from ' + thisTable + ' where ' + thisIdName + ' = ' + thisId, function (error, results, fields) {
      if (error) {
        res.json({type: false, data: error});
      }
      else {
...
app.get('/patientCount', function (req, res) {

   connection.query('SELECT COUNT(*) FROM patients', function (error, results, fields) {
          if (error) throw error;
          res.end(JSON.stringify(results));
        });
...
//SEARCH patients
//app.get('/patientLike/:searchString', function (req, res) {
//   var searchName = req.params.searchString;
//   connection.query('SELECT * FROM patients WHERE patientName LIKE %"' + searchName + '"%', function (error, results, fields) {
//          if (error) throw error;
//          res.end(JSON.stringify(results));
//        });
//});
